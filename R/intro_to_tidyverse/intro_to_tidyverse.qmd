...
# non-tidyverse way: 
mean(penguins[penguins$species=="Adelie",]$bill_length_mm, na.rm=T)
```
...
*Yes! Because each line only has the result from 1 trial (1 image name). Nice job Martin :) *
...
*Data points belonging to each participant will not be independent, so it's best to calculate the average per participant as an index.*
...
*Because we want to immediately see how each participants' performance changes across blocks. This way allows the resulting table to be visually inspected easily.*
...
subj_block_1a
 ...
 nameability_data |>
  filter(experiment == "1A") |>
  group_by(condition,block_num,subject) |>
  summarize(
    accuracy=mean(is_right)
  )
```
